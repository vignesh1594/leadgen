...
usermasterApp.controller('usermasterController', function ($scope, $http, $window) {
    ...
    $scope.addusermaster = function () {
        $http({
            method: 'POST'
            , url: 'http://localhost:8327/api/addusermaster/'
    ...
    $scope.editusermaster = function () {
        $http({
            method: 'POST'
            , url: 'http://localhost:8327/api/editusermaster/'
    ...
    $scope.deleteusermaster = function (userid) {
        ...
        if (result) {
            $http({
                method: 'DELETE'
                , url: 'http://localhost:8327/api/deleteusermaster/' + userid
                , dataType: 'jsonp'
            }).success(function (data) {
                if (!data.success) {
                    $scope.formMessage = data.message;
    ...
    $scope.getusermaster = function (userid) {
        $http({
            method: 'GET'
            , url: 'http://localhost:8327/api/getusermaster/' + userid
            , dataType: 'jsonp'
    ...
    $scope.listusermaster = function () {
        $http({
            method: 'GET'
            , url: 'http://localhost:8327/api/listusermaster'
            , dataType: 'jsonp'
        }).then(function (response) {
	...
	$scope.getusertypes = function () {
        $http({
            method: 'GET',
			url: 'http://localhost:8327/api/getusertypes', 
			dataType: 'jsonp'
        }).then(function (response) {
            $scope.getusertypesdata = response.data;
	...
	$scope.getteamleaders=function(){
		$http({
            method: 'GET', 
			url: 'http://localhost:8327/api/getteamleaders', 
			dataType: 'jsonp'
        }).then(function (response) {
            $scope.getteamleaderdata = response.data;
        });
	}
